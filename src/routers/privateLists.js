...
router.post("/api/private-lists", authenticate, async (req, res) => {
    try {
...
// working with this
router.patch("/api/private-lists/:id", authenticate, async (req, res) => {
...
router.post("/api/private-lists/:id/items", authenticate, async (req, res) => {
    try {
        const list = await PrivateLists.findById(req.params.id);
        if (list === null) {
            return res.status(404).send();
